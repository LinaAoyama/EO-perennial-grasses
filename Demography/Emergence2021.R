...
#Load data
library(tidyverse)
emergence <- read_csv(paste(datpath, "/Demography/Emergence_06182021.csv", sep=""))
library(ggplot2)
...
emergence_summary <- emergence %>%
  ...
  gather(key = "Time", value = "Emergence", March, April, May, June)%>%
...
emergence_summary$Time <- ordered(emergence_summary$Time, levels = c("March", "April", "May", "June"))
